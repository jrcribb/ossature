...
from ntt.models.plan import PlanTask
...
def compute_input_hash(prompt: str, task: PlanTask, config: NTTConfig) -> str:
...
def compute_output_hash(written_files: list[str], config: NTTConfig) -> str:
...
def get_task_written_files(task: PlanTask, tasks_dir: Path) -> list[str]:
    ...
    from ntt.build.builder import make_task_slug
...
def write_state(state: BuildState, filepath: Path) -> None:
    ...
    with open(filepath, "w") as f:
        f.write("# .ntt/state.toml — Build state (auto-generated, do not edit)\n\n")
        f.write(content)
