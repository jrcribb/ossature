...
def run_status(
    ...
    if not plan:
        console.print("[yellow]No build plan found.[/] Run [cyan]ntt audit[/] first.")
        return
    ...
    if failed_task:
        ...
        console.print(
            "  Run [cyan]ntt retry[/] to re-attempt, "
            f"or [cyan]ntt build --spec {failed_task.spec}[/] for targeted rebuild"
        )
